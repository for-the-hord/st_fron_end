  ...
  methods: {
    ...
    //lds   获取流数据转成 luckysheet 可以直接加载的表格
      const all = await axios({
        method: 'post',
        url: `http://localhost:8080/api/getexcel`,
        // data:{id:"12"},
        responseType: 'blob',
      });
    ...
    //     this.turn_file = sessionStorage.getItem('final_blob')
    //    console.log(  this.turn_file ,'sessionSto  this.turn_filerage');
      ...
      formData.append('name','小寒234');
      formData.append('equipment_name', ['890']);
    ...
    //     { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }
    //   );
    //    this.send_file = file  // 转成可以发送的file
    //   console.log('====send_file====', file);
    ...
    //    this.turn_file = sessionStorage.getItem('files')
      ...
      // setTimeout(() => {
    //     let new_blobs = localStorage.getItem('final_blob');
    //   console.log(new_blobs, 'new_blobs');
    // }, 1000);
 //   this.new_all

    //   var blob = new Blob([new_blobs], {
    //     type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    //   });
    //    console.log(blob, '====blob====');
    //   const file = new File(
    ...
    //   this.formInline3.formwork = this.send_file;
    ...
    //lds 这里是用来转换数据的
    exportSheetExcel_final_blob_vue(luckysheet, name) { // 参数为luckysheet.getluckysheetfile()获取的对象
    // 1.创建工作簿，可以为工作簿添加属性

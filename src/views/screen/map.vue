...
// mapsA
import { getMaps, setnewmap } from "./big.js";
export default {
  name: "page6",
  data() {
    return {
  ...
  methods: {
    gomap() {
      getMaps().then((res) => {
        ...
        this.max = res.data.maps[0].value;
        this.chinaConfigure(this.value);
      });
    },
    chinaConfigure(area) {
      ...
      let option = {
        ...
        tooltip: {
          trigger: "item",
          color: "#fff",
          backgroundColor: "#fff0", //标题背景色
          borderColor: "#fff0", //边框颜色
          borderWidth: 1, //边框线宽
          ...
          formatter: function (params) {
            ...
            value =
              ...
              value[1];
            return (
              '<div class="forbg" style="display: flex;align-items: center;justify-content: space-around;width:160px;height:120px;background:url(/image/vmind/tan.png) center no-repeat ;background-size:100%   ">事件总数：' +
              params.value +
              " <br/> 关联漏洞：" +
              params.data.leak +
              " <br/> 影响产品：" +
        ...
        series: [
          {
            ...
            data: [
              { name: "中国", selected: true }, // selected:true 默认选中
            ],
...
.forbg {
  width: 100px;
  height: 90px;
  //   background: url("/image/vmind/dian.png");
}
</style>
